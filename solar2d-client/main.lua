-- main.lua — bulletproof WS + manual transport + HTTP-poll fallback (Solar2D)
...
-- CONFIG
----------------------------------------------------------------
local HOST = "192.168.1.xx"   -- change to your PC LAN IP on device
local PORT = 8080
...
local wsPluginOk, WebSockets = pcall(require, "plugin.websockets")
local wsTryIndex, wsBackoffMs = 0, 600  -- grows on failures
...
-- UTIL / LOGGING / TOAST
----------------------------------------------------------------
local function safeEncode(t) local ok,txt=pcall(json.encode,t) return ok and txt or "null" end
local function safeDecode(s)
  if type(s)~="string" or #s==0 then return nil end
  local ok,obj=pcall(json.decode,s); return ok and obj or nil
end

local logBox
local function log(...)  -- scrollable console + print
  local parts={}
  for i=1,select("#", ...) do
    local v=select(i, ...)
...
local function toast(text)
  ...
  local t=display.newText({text=tostring(text or ""), x=display.contentCenterX,
                           y=display.contentCenterY-130, font=native.systemFontBold,
                           fontSize=14, width=w-24, align="center"})
  t:setFillColor(1,1,1)
  local r=display.newRoundedRect(t.x,t.y, math.max(80,t.width+24), t.height+16, 10)
  r:setFillColor(0,0,0); r.alpha=0.82
  g:insert(r); g:insert(t)
...
local function newBtn(label, centerY, onTap)
  ...
  r:setFillColor(0.20,0.38,0.75); g:insert(r); g:insert(t)
  function g:setEnabled(b) g.enabled=b and true or false; g.alpha=g.enabled and 1 or 0.45 end
  local touching=false
  g:addEventListener("touch",function(e)
    if e.phase=="began" then display.getCurrentStage():setFocus(g); touching=true; g.xScale,g.yScale=0.98,0.98
    elseif e.phase=="ended" or e.phase=="cancelled" then
      display.getCurrentStage():setFocus(nil); g.xScale,g.yScale=1,1
...
-- header + console (custom bg to avoid Windows textbox warnings)
do
  local sx,sy,sw,sh=SafeArea()
  local title=display.newText({text="Solar2D ↔ Java Backend",x=display.contentCenterX,y=sy+22,
                               font=native.systemFontBold,fontSize=18})
  local sub=display.newText({text=("%s  •  %s"):format(BASE,WSURL),x=display.contentCenterX,y=title.y+18,
  ...
  bg:setFillColor(0.08,0.08,0.1); bg.alpha=0.9

  logBox=native.newTextBox(display.contentCenterX, sy+sh-120, sw-24, 172)
  logBox.isEditable=false; logBox.size=12
...
local function setWs(ok)   wsBadge:set(ok); wsOpen = ok and true or false end
local function setModeText() modeText.text = transportMode end

-- tapping the mode label cycles transport: AUTO → WS → POLL
modeText:addEventListener("tap", function()
  ...
  if polling then polling=false; if pollTimer then timer.cancel(pollTimer); pollTimer=nil end end
  if ws and ws.close then pcall(function() ws:close() end) end
  setWs(false)
  return true
end)
setModeText()
...
local function http(method, path, body, cb)
  if type(path)~="string" then log("HTTP path not string; abort"); return end
  local url=BASE..path
  ...
  network.request(url, method, function(event)
    if event.isError then
      log("HTTP error",method,path,"status:",tostring(event.status)); setHttp(false)
      if cb then cb(nil,event) end; return
    end
    setHttp(true)
...
local function stopPolling()
  polling=false
  if pollTimer then timer.cancel(pollTimer); pollTimer=nil end
end
...
local function wsUnifiedHandler(e)
  ...
  if     t=="open"    then
    stopPolling(); setWs(true); wsBackoffMs = 600
    log("WS open"); toast("WebSocket connected")
  ...
  elseif t=="close" then
    setWs(false); log("WS closed", e and e.code or "", e and e.reason or "")
    if transportMode~="WS" then startPolling() end
  elseif t=="error" then
    setWs(false); log("WS error:", (e and (e.errorMessage or e.reason)) or "unknown")
    if transportMode~="WS" then startPolling() end
  else
...
local function hookWsEvents()
  ...
  if WebSockets and WebSockets.EVENT then
    ws:addEventListener(WebSockets.EVENT, wsUnifiedHandler)
  else
...
-- Strategies seen across plugin versions:
-- 1) new(WSURL); connect()
-- 2) new();      connect(WSURL)
...
local function connectWithStrategy(idx)
  ...
  local okNew, inst
  if mode==1 then
    okNew,inst=pcall(function() return WebSockets.new(WSURL) end)
  elseif mode==2 or mode==4 or mode==5 then
    okNew,inst=pcall(function() return WebSockets.new() end)
  elseif mode==3 then
    okNew,inst=pcall(function() return WebSockets.new({url=WSURL}) end)
  end
  if not okNew or not inst then
    ...
    if idx<#STRATS then
      ...
      log("WS: all strategies failed"); setWs(false)
      if transportMode~="WS" then startPolling() end
      return
    end
  end

  ws=inst; hookWsEvents()
  ...
  -- If no 'open' in 1.6s, try the next one (or backoff + poll)
      ...
      if idx<#STRATS then
        ...
        if transportMode~="POLL" then
          timer.performWithDelay(wsBackoffMs, function()
            ...
            if transportMode=="AUTO" then startPolling() end  -- keep realtime via HTTP while retrying
...
local function openWS()
  ...
  if not wsPluginOk then
    ...
    if transportMode=="WS" then toast("WS plugin missing"); return end
    startPolling(); return
  end
  if wsOpen then log("WS already open"); return end
  connectWithStrategy(1)
end
...
local btnLogin = newBtn("1) Login",          y, doLogin);       y=y+48
local btnWS    = newBtn("2) Connect",        y, openWS);        y=y+48
local btnSave  = newBtn("3) Save Progress",  y, saveProgress);  y=y+48
local btnBC    = newBtn("4) Broadcast Test", y, broadcastTest)
